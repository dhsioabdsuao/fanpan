...
数据来源：Apple 官方 [截图规范文档](https://developer.apple.com/help/app-store-connect/reference/screenshot-specifications)。

**只需上传一个 6.9 英寸尺寸，其余由 Apple 自动缩放生成。**

| 显示尺寸 | 竖屏分辨率 | 是否必需 |
|----------|-----------|----------|
| **6.9"** | **1290 × 2796 px** | ✅ **必需**（三选一：1260×2736 / 1290×2796 / 1320×2868） |
| 6.5" | 1284 × 2778 px | ⚠️ 仅当未提供 6.9" 时必需 |
| 6.3" | 1206 × 2622 px | 可选，否则从 6.5" 缩放 |
| 6.1" | 1170 × 2532 px | 可选，否则从 6.5" 缩放 |
| 5.5" | 1242 × 2208 px | 可选，否则从 6.1" 缩放 |
| 4.7" | 750 × 1334 px | 可选，否则从 5.5" 缩放 |

**格式要求：**
- 文件格式：JPEG、JPG、PNG（不含 alpha 通道）
...
```bash
cd bazi-app-sdk52
npx expo run:ios   # 在 6.9" 模拟器（如 iPhone 16 Pro Max）中运行
# 模拟器菜单 → File → Save Screenshot（或 Cmd+S）
```

模拟器截图即为设备原生 1290×2796 px，无需缩放。

**截图规范：**
- 状态栏时间设为 9:41，信号/Wi-Fi/电量满格
- 不要包含个人敏感信息
- 推荐使用模拟数据（如 1990-01-01 的八字）
- 确保中文文字清晰可读
