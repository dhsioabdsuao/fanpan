...
const FILE_PATH = new File(Paths.document, 'readings.json');
...
export async function loadRecords(): Promise<SavedRecord[]> {
  try {
    ...
    const data = await FILE_PATH.text();
...
export async function saveRecord(
  ...
  try {
    ...
    FILE_PATH.write(JSON.stringify(trimmed));
  } catch {
    // 静默失败，不影响排盘
  }
}
...
export async function deleteRecord(id: string): Promise<void> {
  try {
    ...
    FILE_PATH.write(JSON.stringify(filtered));
  } catch {
    // 静默失败
  }
}
